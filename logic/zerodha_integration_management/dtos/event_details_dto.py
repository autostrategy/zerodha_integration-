from pydantic import BaseModel
from datetime import datetime
from typing import Optional

from logic.zerodha_integration_management.dtos.tp_details_dto import TPDetailsDTO


class EventDetailsDTO(BaseModel):
    alert_time: Optional[datetime]
    symbol: Optional[str]
    time_frame: Optional[str]
    trade1_quantity: Optional[int]
    event1_occur_time: Optional[datetime]
    event2_occur_time: Optional[datetime]
    event3_occur_time: Optional[datetime]
    event2_occur_breakpoint: Optional[float]
    event3_occur_breakpoint: Optional[float]
    current_candle_high: Optional[float]
    current_candle_low: Optional[float]
    candle_length: Optional[float]
    combined_height: Optional[float]
    highest_point: Optional[float]  # For 'SELL' part
    lowest_point: Optional[float]
    event1_candle_high: Optional[float]
    adjusted_high: Optional[float]
    event1_candle_low: Optional[float]
    adjusted_low: Optional[float]  # For 'SELL' part
    high_1: Optional[float]
    low_1: Optional[float]  # For 'SELL' part
    sl_value: Optional[float]
    tp_values: Optional[list[TPDetailsDTO]] = []
    sl_hit: bool = False
    tp_hit: bool = False
    sl_candle_time: Optional[datetime]
    tp_candle_time: Optional[datetime]
    entry_trade_order_id: Optional[str]
    actual_calculated_trade1_quantity: Optional[int]
    actual_calculated_extension1_quantity: Optional[int]
    actual_calculated_extension2_quantity: Optional[int]
    entry_trade_datetime: Optional[datetime]
    sl_order_id: Optional[str]
    tp_order_id: Optional[str]
    tp_order_status: Optional[str]
    sl_order_status: Optional[str]
    entry_price: Optional[float]
    extension1_trade_order_id: Optional[str]
    extension1_trade_datetime: Optional[datetime]
    extension2_trade_order_id: Optional[str]
    extension2_trade_datetime: Optional[datetime]
    extension_quantity: Optional[int]
    value_to_add_for_stop_loss_entry_price: Optional[float]
    sl_buffer: Optional[float]
    previous_timestamp: Optional[datetime]
    extension1_quantity: Optional[int]
    extension2_quantity: Optional[int]
    extension1_entry_price: Optional[float]
    extension2_entry_price: Optional[float]
    extended_sl: Optional[float]
    cover_sl: Optional[float]
    cover_sl_quantity: Optional[int]
    cover_sl_trade_order_id: Optional[str]
    sl_trade_datetime: Optional[datetime]
    reverse_trade_stop_loss: Optional[float]
    reverse_trade_take_profit: Optional[float]
    value_to_add_to_reverse_trade_take_profit: Optional[float]
    reverse1_entry_price: Optional[float]
    reverse1_trade_order_id: Optional[str]
    reverse1_trade_datetime: Optional[datetime]
    reverse1_trade_quantity: Optional[int]
    reverse2_entry_price: Optional[float]
    reverse2_trade_order_id: Optional[str]
    reverse2_trade_datetime: Optional[datetime]
    reverse2_trade_quantity: Optional[int]
    total_reverse_trade_quantity: Optional[int]
    reverse_trade_tp_order_id: Optional[str]
    reverse_trade_tp_order_datetime: Optional[datetime]
    reverse_trade_sl_order_datetime: Optional[datetime]
    reverse_cover_sl_trade_order_id: Optional[str]
    reverse_cover_sl_trade_datetime: Optional[datetime]
    reverse_cover_sl_quantity: Optional[int]
    reverse_trade_sl_order_id: Optional[str]
    reverse_trade_sl_order_status: Optional[str]
    reverse_trade_tp_order_status: Optional[str]
    stop_tracking_further_events: Optional[bool]
    range_issue: Optional[bool]
    extended_sl_timestamp: Optional[str]
